...
However, instead of sampling from $\pi(\theta,G|X)$ directly, we employ Gibbs sampling \citep{gelfand_sampling-based_1990} and alternately sample from $\pi(\theta|G,X)$ and $\pi(G|\theta,X)$.
...
%Lacking a way to determine this value directly, one may exploit the following equation. 
...
\note{TODO: Low-throughput experiments have yet to be implemented}%TODO <--
